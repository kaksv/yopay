...
function Hero() {
  ...
  const { writeContract, isPending } = useWriteContract({
    mutation: {
      onSuccess: (data) => {
        setTxHash(data) // Store hash in txHash
        setTxStatus(data.hash);
      },
      onError: (error) => {
  ...
  const handleSendTokens = async () => {
    if (!recipientAddress || !amount) {
     ...
     writeContract({
      ...
      // address: chainId === sepolia.id ? sepoliaUsdcAddress : baseSepoliaUsdcAddress,
      abi: tokenAbi,
      functionName: 'transfer',
      args: [recipientAddress, ethers.parseUnits(amount, 6)], // ethers.js v6
    });
      ...
      <div className="bg-[#eaeaea] p-6 rounded-lg  w-full max-w-md">
          ...
          {isConnected ? (
            ...
            <div className="bg-white p-2 rounded-md shadow-sm">
              <div className="flex justify-between items-center">
                 {/* alert("You are connected to Lisk Mainnet"); */}
              ...
              <div className="flex justify-between items-center mt-1">
                ...
                {/* <button onClick={() => disconnectWallet()} className="bg-gray-100 px-2 py-1 rounded text-sm">
                  Disconnect
                </button> */}
              </div>
            </div>
          ) : (
            <w3m-button /> /* Use the Web3Modal Button component */
          )}
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="flex gap-2">
            ...
            >
              {/* <option value={sepolia.id}>Ethereum Sepolia</option> */}
              ...
              <option value={celoAlfajores.id}> Celo Alfajores</option> */}
            </select>
          </div>
        </div>
        ...
        {txHash && (
          <div className="mt-4 text-sm text-gray-600">
            ...
            <p>
              <a href={`https://blockscout.lisk.com/tx/${txHash}`}
               target="_blank"
            ...
            <p>
              ...
              <button
                onClick={() => copyToClipboard(txStatus)}
                className="text-blue-500 hover:underline"
              >
                Copy
              </button>
